...
void IWRAM_CODE SetRompageWithHardReset(u16 page, u32 bootmode)
{
	...
	SetRompage(page);
	RegisterRamReset(RESET_EWRAM | RESET_PALETTE | RESET_VRAM | RESET_OAM |
			 RESET_SIO | RESET_SOUND | RESET_OTHER);
	if (bootmode == 1) {
		HardReset();
	} else {
		SoftReset_now();
	...
	*((vu16 *) (FlashBase_S71 + offset)) = 0x30;	//erase
	...
	for (loopwrite = 0; loopwrite < (buffersize / 32); loopwrite++) {
		...
		*((vu16 *) (FlashBase_S71 + offset + loopwrite * 32)) = 15;
		for (i = 0; i <= 15; i++) {
			*((vu16 *) (FlashBase_S71 + offset + loopwrite * 32 +
				    2 * i)) = buf[loopwrite * 16 + i];
		}
...
void IWRAM_CODE Read_NOR_info()
{
	...
	for (loopwrite = 0; loopwrite < sizeof(FM_NOR_FS) * 0x40; loopwrite++) {
		((u16 *) pNorFS)[loopwrite] =
		    *((vu16 *) (FlashBase_S71 + NOR_info_offset +
				loopwrite * 2));
	}
}
...
void IWRAM_CODE Check_FW_update(u16 Current_FW_ver, u16 Built_in_ver)
{
	...
	if (get_crc32 != 0xEFD03788)	//fw7
	{
		...
		DrawHZText12(msg, 0, 2, offset_Y + 0 * line_x, RGB(31, 00, 00),
			     1);
	...
	while (1) {
		...
		if (keys & KEY_A) {
			SPI_Write_Disable();
			Clear(2, offset_Y + 4 * line_x, 220, 15, RGB(0, 18, 24),
			      1);
			Clear(2, offset_Y + 5 * line_x, 220, 15, RGB(0, 18, 24),
			      1);

			sprintf(msg, "progress:");
			DrawHZText12(msg, 0, 2, offset_Y + 6 * line_x, 0x7FFF,
				     1);

			for (offset = 0x0000; offset < newomega_top_bin_size;
			     offset += 256) {

				sprintf(msg, " %lu%%",
					(offset * 100 / newomega_top_bin_size +
					 1));
				Clear(54, offset_Y + 6 * line_x, 120, 15,
				      RGB(0, 18, 24), 1);
				DrawHZText12(msg, 0, 54, offset_Y + 6 * line_x,
					     0x7FFF, 1);

				FAT_table_buffer[0] = (0x40000 + offset);

				dmaCopy(newomega_top_bin + offset,
					&FAT_table_buffer[1], 256);
			...
			DrawHZText12(msg, 0, 2, offset_Y + 8 * line_x, 0x7FFF,
				     1);
