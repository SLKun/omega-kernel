	...
	if ((blockAdd == 0) || (blockAdd == 0x3FE0000)) {
		...
		for (loop = 0; loop < 0x20000; loop += 0x8000) {
			...
			*((vu16 *) (FlashBase_S98 + Address + loop)) = 0x30;
			do {
				v1 = *((vu16 *) (FlashBase_S98 + Address +
						 loop));
				v2 = *((vu16 *) (FlashBase_S98 + Address +
						 loop));
			} while (v1 != v2);
		}
	} else {
...
void WriteFlash(u32 address, u8 * buffer, u32 size)
{
	...
	for (loopwrite = 0; loopwrite < (size / 2); loopwrite++) {
		...
		*((vu16 *) (FlashBase_S98 + address + loopwrite * 2)) =
		    buf[loopwrite];
		do {
			v1 = *((vu16 *) (FlashBase_S98 + address +
					 loopwrite * 2));
...
void IWRAM_CODE WriteFlash_with32word(u32 address, u8 * buffer, u32 size)
{
	...
	for (loopwrite = 0; loopwrite < (size / 32); loopwrite++) {
		...
		*((vu16 *) (FlashBase_S98 + address + loopwrite * 32)) = 15;
		for (i = 0; i <= 15; i++) {
			*((vu16 *) (FlashBase_S98 + address + loopwrite * 32 +
	...
	if (norid == 0x223D)	//S98
	{
		...
		*((vu16 *) (FlashBase_S98 + 0x000 * 2)) = 0x00;
		...
		for (blocknum = 0; blocknum < filesize; blocknum += 0x20000) {
			...
			Clear(78 + 54, 160 - 15, 100, 15, gl_color_cheat_black,
			      1);
			...
			if (have_patch) {
				if ((gl_reset_on == 1) || (gl_rts_on == 1)
				    || (gl_sleep_on == 1)
				    || (gl_cheat_on == 1)) {
					PatchInternal((u32 *) pReadCache,
						      0x20000, blocknum);
					GBApatch_NOR((u32 *) pReadCache, 0x20000, blocknum);	//some nes need check                           
				}
			} else {
				GBApatch_Cleanrom_NOR((u32 *) pReadCache,
						      blocknum);
			}

			WriteFlash_with32word(blocknum + NORaddress, pReadCache,
					      0x20000);
		...
		if (have_patch) {
			if (add_patch) {
				Block_Erase(blocknum + NORaddress);
				GBApatch_NOR((u32 *) pReadCache, 0x20000,
					     blocknum);
				WriteFlash_with32word(blocknum + NORaddress,
						      pReadCache, 0x20000);
			}
		}
...
u32 GetFileListFromNor(void)
{
	...
	while (((Value & 0xFF) == 0xCE) || ((Value & 0xFF) == 0xCF)
	       || ((Value & 0xFF) == 0x00) || (x24 == 0x51ae)) {
		//DEBUG_printf(" %x %x %x",StartAddress,Value,x24);
