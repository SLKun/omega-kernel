...
//------------------------------------------------------------------
void Get_KEY_val(FIL * file, char *KEY_section, char *KEY_secval,
		 char getbuff[])
{
	int text_comment = 0;
...
//------------------------------------------------------------------
u32 Get_CHT_val(FIL * file, char *KEY_section,
		char *KEY_secval /*,char getbuff[] */ )
{
	int text_comment = 0;
...
u32 Get_all_Section_val(FIL * file)
{
	...
	while (f_gets(buf, MAX_sectionVAL_LEN, &gfile) != NULL) {
		...
		for (i = 0; i < buf_len; ++i) {
			...
			if (in_section == 1) {
				...
				if (_kv == 0 && buf[i] != '=') {
					...
					memcpy(tmpCHTFS.LINEname, _paramk,
					       _klen);
		...
		if (_kv == 1) {
			...
			dmaCopy(&tmpCHTFS, &((FM_CHT_LINE *) pCHTbuffer)[Line],
				sizeof(FM_CHT_LINE));
			Line++;
		}
	}
...
void Show_KEY_val(u32 total, u32 Select, u32 showoffset)
{
	...
	for (line = 0; line < need_show; line++) {
		...
		u8 select =
		    ((FM_CHT_LINE *) pCHTbuffer)[showoffset + line].select;

		if (((FM_CHT_LINE *) pCHTbuffer)[showoffset +
						 line].is_section == 1) {
			Clear(X_offset + 3, Y_offset + line * line_x + 4, 4, 4, gl_color_text, 1);	//section flag

			sprintf(msg, "%s",
				((FM_CHT_LINE *) pCHTbuffer)[showoffset +
							     line].LINEname);
			...
			DrawHZText12(msg, 30, X_offset + 13,
				     Y_offset + line * line_x, name_color, 1);
		} else {
			VBlankIntrWait();
			Draw_select_icon(X_offset + 13,
					 Y_offset + line * line_x, select);
			...
			DrawHZText12(msg, 30, X_offset + 15 + 13,
				     Y_offset + line * line_x, name_color, 1);
...
void Analyze_KEYVAL(FIL * file, u32 total)
{
	...
	for (tol = 0; tol < total; tol++) {
		...
		if (((FM_CHT_LINE *) pCHTbuffer)[tol].select == 1) {
			...
			while (((FM_CHT_LINE *) pCHTbuffer)[current_select].
			       is_section != 1) {
				current_select--;
			}
			...
			buflen =
			    Get_CHT_val(&gfile,
					((FM_CHT_LINE *)
					 pCHTbuffer)[current_select].LINEname,
					((FM_CHT_LINE *) pCHTbuffer)[tol].
					LINEname);
			...
			for (i = 0; i < buflen; i++) {
				if (_paramv[i] == ' ') {
					...
					if (is_address == 1)	//first ','
					{
						...
						ST_entry cheat =
						    { str2hex(address_buf) +
					       address_add, str2hex(val_buf) };
						pCHEAT[gl_cheat_count++] =
						    cheat;
			...
			ST_entry cheat =
			    { str2hex(address_buf) + address_add,
		       str2hex(val_buf) };
			pCHEAT[gl_cheat_count++] = cheat;
...
u32 Check_count(u32 all_count)
{
	...
	for (Line = 0; Line < all_count; Line++) {
		if (strcmp
		    (((FM_CHT_LINE *) pCHTbuffer)[Line].LINEname,
		     "GameInfo") == 0)
			break;
...
u32 Change2cht_folder(u32 chtname)
{
	...
	sprintf(chtnamebuf, "%d%d%d%d", HexToChar(((u8 *) & chtname)[0]),
		HexToChar(((u8 *) & chtname)[1]),
		HexToChar(((u8 *) & chtname)[2]),
		HexToChar(((u8 *) & chtname)[3]));
			...
			for (i = 0; i < filesize / 4; i += 2) {
				...
				if (tempbuff[i] == GAMEID) {
					...
					if (res != 0)
						return 0;
					memset(chtnamebuf, 0x00, 100);
					sprintf(chtnamebuf, "%d%d%d%d.cht",
						HexToChar(((u8 *) &
							   chtname)[0]),
						HexToChar(((u8 *) &
							   chtname)[1]),
...
void Open_cht_file(TCHAR * gamefilename, u32 havecht)
{
	...
	if (havecht == 0x0000FFFF) {
		...
		u8 *chtmode;
		chtmode = &havecht;
		sprintf(chtnamebuf, "%d%d%d%d.cht", HexToChar(chtmode[0]),
			HexToChar(chtmode[1]), HexToChar(chtmode[2]),
			HexToChar(chtmode[3]));
	}
	...
	if (res == FR_OK)	//have a cht file
	{
		...
		if (all_count) {
			...
			while (1)	//3
			{
				...
				if (re_show) {
					if (re_show > 1) {
						Clear(0, 19, 240, 160 - 19,
						      gl_color_chtBG, 1);
					}
					Show_KEY_val(all_count, Select,
						     showoffset);
					Show_num(all_count,
						 Select + showoffset + 1);
					re_show = 0;
