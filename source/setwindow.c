...
u32 Setting_window(void)
{
	...
	while (1) {
		...
		if (re_show) {
			...
			DrawHZText12(msg, 0, set_offset, y_offset,
				     gl_color_selected, 1);

			//
			sprintf(msg, "%s", gl_addon);
			DrawHZText12(msg, 0, set_offset, y_offset + line_x,
				     gl_color_selected, 1);

			Draw_select_icon(x_offset, y_offset + line_x, v_reset);
			sprintf(msg, "%s", gl_reset);
			DrawHZText12(msg, 0, x_offset + 15, y_offset + line_x,
				     (addon_sel ==
				      0) ? gl_color_selected : gl_color_text,
				     1);

			Draw_select_icon(x_offset + 12 * 6, y_offset + line_x,
					 v_rts);
			sprintf(msg, "%s", gl_rts);
			DrawHZText12(msg, 0, x_offset + 12 * 6 + 15,
				     y_offset + line_x,
				     (addon_sel ==
				      1) ? gl_color_selected : gl_color_text,
				     1);
			VBlankIntrWait();
			Draw_select_icon(x_offset, y_offset + line_x * 2,
					 v_sleep);
			sprintf(msg, "%s", gl_sleep);
			DrawHZText12(msg, 0, x_offset + 15,
				     y_offset + line_x * 2,
				     (addon_sel ==
				      3) ? gl_color_selected : gl_color_text,
				     1);
			VBlankIntrWait();
			Draw_select_icon(x_offset + 12 * 6,
					 y_offset + line_x * 2, v_cheat);
			sprintf(msg, "%s", gl_cheat);
			DrawHZText12(msg, 0, x_offset + 12 * 6 + 15,
				     y_offset + line_x * 2,
				     (addon_sel ==
				      4) ? gl_color_selected : gl_color_text,
				     1);

			//                                      
			sprintf(msg, "%s", gl_language);
			DrawHZText12(msg, 0, set_offset, y_offset + line_x * 3,
				     gl_color_selected, 1);
			Draw_select_icon(x_offset, y_offset + line_x * 3,
					 (language_sel == 0x0));
			Draw_select_icon(x_offset + 12 * 6,
					 y_offset + line_x * 3,
					 (language_sel == 0x1));
			...
			DrawHZText12(msg, 0, x_offset + 15,
				     y_offset + line_x * 3, ((language_sel == 0)
							     && currstate
							     && (2 ==
								 select)) ?
				     gl_color_selected : gl_color_text, 1);
			sprintf(msg, "%s", gl_zh_lang);
			DrawHZText12(msg, 0, x_offset + 12 * 6 + 15,
				     y_offset + line_x * 3, ((language_sel == 1)
							     && currstate
							     && (2 ==
								 select)) ?
				     gl_color_selected : gl_color_text, 1);
			...
			DrawHZText12(msg, 0, set_offset, y_offset + line_x * 4,
				     gl_color_selected, 1);
			Draw_select_icon(x_offset, y_offset + line_x * 4,
					 (engine_sel == 0x1));
			...
			ClearWithBG((u16 *) gImage_SET, set_offset,
				    y_offset + line_x * 5, 9 * 6, 13, 1);
			ClearWithBG((u16 *) gImage_SET, set_offset,
				    y_offset + line_x * 6, 9 * 6, 13, 1);
			if ((v_rts == 1) && (v_cheat == 0) && (v_reset == 0)
			    && (v_sleep == 0)) {
				sprintf(msg, "%s", " SAVE KEY");
				DrawHZText12(msg, 0, set_offset,
					     y_offset + line_x * 5,
					     gl_color_selected, 1);
			...
			DrawHZText12(msg, 0, set_offset, y_offset + line_x * 7,
				     gl_color_selected, 1);
			Draw_select_icon(x_offset, y_offset + line_x * 7,
					 (gl_ingame_RTC_open_status == 0x1));
			//sprintf(msg,"%s",gl_offRTC_powersave);
			ClearWithBG((u16 *) gImage_SET, x_offset + 15,
				    y_offset + line_x * 7, 6 * 6, 13, 1);
			...
			for (line = 0; line < 7; line++) {
				...
				DrawHZText12(msg, 0, 200 + 5, offsety,
					     gl_color_text, 1);
				VBlankIntrWait();
			}
		}
		...
		case 0:	//initial state
			...
			if (SS > 59)
				SS = 0;
			sprintf(msg, "%u/%02u/%02u %02d:%02d:%02d %s",
				UNBCD(datetime[0]) + 2000,
				UNBCD(datetime[1] & 0x1F),
				UNBCD(datetime[2] & 0x3F), HH, MM, SS, wkday);
			ClearWithBG((u16 *) gImage_SET, x_offset, y_offset,
				    22 * 6, 13, 1);
			DrawHZText12(msg, 0, x_offset, y_offset, gl_color_text,
				     1);
			VBlankIntrWait();
			...
			DrawHZText12(msg, 0, x_offset + 10,
				     y_offset + line_x * 5, gl_color_text, 1);
			...
			DrawHZText12(msg, 0, x_offset + 10,
				     y_offset + line_x * 6, gl_color_text, 1);
			...
			if (keys & KEY_A) {	//set
				...
				edit_datetime[_DAY] =
				    UNBCD(datetime[_DAY] & 0x3F);
				edit_datetime[_MONTH] =
				    UNBCD(datetime[_MONTH] & 0x1F);
				edit_datetime[_YEAR] = UNBCD(datetime[_YEAR]);
				edit_datetime[_WKD] =
				    UNBCD(datetime[_WKD] & 0x7);
		...
		case 1:	//edit state    
			...
			if (re_show) {
				if (select == 0) {
					ClearWithBG((u16 *) gImage_SET,
						    x_offset, y_offset, 23 * 6,
						    13, 1);
					switch (edit_pos) {
					case 0:
						clean_pos = x_offset;
					...
					if (edit_pos < 7)
						Clear(clean_pos, y_offset,
						      clean_w, 13,
						      gl_color_btn_clean, 1);
					...
					sprintf(msg,
						...
						edit_datetime[_HOUR],
						edit_datetime[_MIN],
						edit_datetime[_SEC], wkday);
					DrawHZText12(msg, 0, x_offset, y_offset,
						     gl_color_text, 1);
				} else if (select == 4) {
					ClearWithBG((u16 *) gImage_SET,
						    x_offset,
					...
					if (sleep_pos < 3)
						Clear(clean_pos,
						      y_offset + line_x * 5, 36,
						      13, gl_color_btn_clean,
						      1);
			...
			if (keysrepeat & KEY_UP) {
				...
				if ((0 == select) && (edit_pos == 7)) {
					...
					ClearWithBG((u16 *) gImage_SET, 0, 20,
						    240, 160 - 20, 1);
					Set_OK = 0;
				} else if (select == 3) {
					switch (engine_pos) {
