...
void Show_ICON_filename(u32 show_offset, u32 file_select, u32 haveThumbnail)
{
	...
	for (line = 0; line < need_show_folder; line++) {
		...
		if (line == file_select) {
			Clear(17, 20 + file_select * 14,
			      (char_num == 17) ? (17 * 6 + 1) : (240 - 17), 13,
			      gl_color_selectBG_sd, 1);
		}

		DrawPic((u16 *) (gImage_icons + 0 * 16 * 14 * 2),
			0, y_offset + line * 14, 16, 14, 1, gl_color_text, 1);

		DrawHZText12(pFolder[show_offset + line].filename, char_num,
			     1 + 16, y_offset + line * 14, name_color, 1);

		if ((haveThumbnail == 1) && (line > 3)) {
		} else {
			char msg[20];
			sprintf(msg, "%s", "DIR");
			DrawHZText12(msg, 0, 221, y_offset + line * 14,
				     name_color, 1);
		}
	}
	...
	for (line = need_show_folder; line < need_show_folder + need_show_game;
	     line++) {
		if (haveThumbnail) {
			if (line > 3) {
				char_num = 17;
		...
		pfilename =
		    pFilename_buffer[offset + line - need_show_folder].filename;
		...
		DrawHZText12(pFilename_buffer[offset + line - need_show_folder].
			     filename, char_num, 1 + 16, showy, name_color, 1);
		if ((haveThumbnail == 1) && (line > 3)) {
		} else {
			char msg[20];
...
//---------------------------------------------------------------------------------
void IWRAM_CODE Refresh_filename(u32 show_offset, u32 file_select, u32 updown,
				 u32 haveThumbnail)
{
	u32 need_show_game;
	u32 need_show_folder;
	...
	if (updown == 2)	//down
	{
		...
		ClearWithBG((u16 *) gImage_SD, 17, 20 + xx1 * 14, clean_len1,
			    13, 1);
		Clear(17, 20 + xx2 * 14, clean_len2, 13, gl_color_selectBG_sd,
		      1);
	} else			// if(updown ==3)//up
	{
		xx1 = file_select;
		...
		Clear(17, 20 + xx1 * 14, clean_len1, 13, gl_color_selectBG_sd,
		      1);
		ClearWithBG((u16 *) gImage_SD, 17, 20 + xx2 * 14, clean_len2,
			    13, 1);
	}

	if ((file_select == (need_show_folder - 1)) && (updown == 3)) {
		DrawHZText12(pFolder[show_offset + xx1].filename, char_num1,
			     1 + 16, showy1, name_color1, 1);
		DrawHZText12(pFilename_buffer[0].filename, char_num2, 1 + 16,
			     showy2, name_color1, 1);
		...
		DrawHZText12(pFolder[show_offset + xx1].filename, char_num1,
			     1 + 16, showy1, name_color1, 1);
		DrawHZText12(pFolder[show_offset + xx2].filename, char_num2,
			     1 + 16, showy2, name_color1, 1);
		...
		DrawHZText12(pFilename_buffer[offset + xx1 - need_show_folder].
			     filename, char_num1, 1 + 16, showy1, name_color1,
			     1);
		DrawHZText12(pFilename_buffer[offset + xx2 - need_show_folder].
			     filename, char_num2, 1 + 16, showy2, name_color1,
			     1);
...
void Show_ICON_filename_NOR(u32 show_offset, u32 file_select)
{
	...
	for (line = 0; line < need_show; line++) {
		if (line == file_select) {
			Clear(17, 20 + file_select * 14, 240 - 17, 13,
			      gl_color_selectBG_nor, 1);
		}

		DrawPic((u16 *) gImage_nor_icon /*(gImage_icons+2*16*14*2) */ ,
			0, y_offset + line * 14, 16, 14, 1, gl_color_text, 1);

		DrawHZText12(pNorFS[show_offset + line].filename, char_num,
			     1 + 16, y_offset + line * 14, name_color, 1);
		sprintf(msg, "%4luM",
			pNorFS[show_offset + line].filesize >> 20);
...
void Refresh_filename_NOR(u32 show_offset, u32 file_select, u32 updown)
{
	...
	if (updown == 2)	//down
	{
		...
		showy1 = y_offset + (file_select - 1) * 14;
		showy2 = y_offset + (file_select) * 14;
		ClearWithBG((u16 *) gImage_NOR, 17, 20 + xx1 * 14, clean_len,
			    13, 1);
		Clear(17, 20 + xx2 * 14, clean_len, 13, gl_color_selectBG_nor,
		      1);
	} else			//if(updown ==3)//up
	{
		xx1 = file_select;
		...
		showy1 = y_offset + (file_select) * 14;
		showy2 = y_offset + (file_select + 1) * 14;
		Clear(17, 20 + xx1 * 14, clean_len, 13, gl_color_selectBG_nor,
		      1);
		ClearWithBG((u16 *) gImage_NOR, 17, 20 + xx2 * 14, clean_len,
			    13, 1);
	}

	DrawHZText12(pNorFS[show_offset + xx1].filename, char_num, 1 + 16,
		     showy1, name_color1, 1);
	DrawHZText12(pNorFS[show_offset + xx2].filename, char_num, 1 + 16,
		     showy2, name_color1, 1);
...
void Show_game_num(u32 count, u32 list)
{
	...
	if (list == 0) {
		if (game_total_SD + folder_total == 0)
			count = 0;
		sprintf(msg, "[%03lu/%03lu]", count,
			game_total_SD + folder_total);
	} else {
		if (game_total_NOR == 0)
			count = 0;
...
//---------------------------------------------------------------------------------
void Filename_loop(u32 shift, u32 show_offset, u32 file_select,
		   u32 haveThumbnail)
{
	u32 need_show_folder;
	//u32 line;
	...
	if (shift > timeout) {
		...
		if (file_select < need_show_folder) {
			strncpy(temp_filename,
				pFolder[show_offset + file_select].filename,
				100);

		} else {
			strncpy(temp_filename,
				pFilename_buffer[offset + file_select -
						 need_show_folder].filename,
				100);
		}
		...
		if (namelen > (char_num - 1)) {
			...
			if (orgtt != tt) {
				...
				strncpy(msg + strlen(msg), temp_filename,
					128 - strlen(msg));
				...
				Clear(17, 20 + file_select * 14, (char_num) * 6,
				      13, gl_color_selectBG_sd, 1);
				DrawHZText12(msg, char_num - 1, 1 + 16,
					     y_offset + file_select * 14,
					     gl_color_text, 1);
			}
		}
	}
...
//---------------------------------------------------------------------------------
void Show_MENU(u32 menu_select, PAGE_NUM page, u32 havecht, u32 Save_num,
	       u32 is_menu)
{
	int line;
	u32 y_offset = 30;
	...
	for (line = 0; line < linemax; line++) {
		...
		if (page == NOR_list)
			DrawHZText12(gl_nor_op[line], 32, 60,
				     y_offset + line * 14, name_color, 1);
		else {
			if (line == 5)	//cheat
			{
				sprintf(msg, "%s(%d)", gl_rom_menu[line],
					gl_cheat_count);
				DrawHZText12(msg, 32, 60, y_offset + line * 14,
					     name_color, 1);
			} else {
				DrawHZText12(gl_rom_menu[line], 32, 60,
					     y_offset + line * 14, name_color,
					     1);
...
void Show_game_name(u32 total, u32 Select)
{
	...
	for (line = 0; line < need_show; line++) {
		...
		DrawHZText12(msg, 39, X_offset, Y_offset + line * line_x,
			     name_color, 1);
	...
	for (i = 0; i < count; i++) {
		...
		if (get == 0) {
			...
			for (j = i; j > 0; j--) {
				...
				dmaCopy(&(p_recently_play[j - 1]),
					&(p_recently_play[j]), 512);
			}
			break;
		}
	}
	...
	if (get != 0) {
		if (count == 10) {
			for (i = 9; i > 0; i--) {
				...
				dmaCopy(&(p_recently_play[i - 1]),
					&(p_recently_play[i]), 512);
			}
		} else if (count) {
			for (i = count; i > 0; i--) {
				memset(p_recently_play[i], 0x00, 512);
...
void init_FAT_table(void)
{
	...
	CpuFastSet(FAT_table_buffer, FAT_table_buffer,
		   FILL | (FAT_table_size / 4));
	...
	case FR_OK:
		{
			...
			if (filesize > 64 * 1024) {
				f_read(&file, pReadCache, 64 * 1024,
				       (UINT *) & ret);
				...
				f_read(&file, pReadCache, filesize,
				       (UINT *) & ret);
	...
	case FR_OK:
		{
			...
			if (savesize < 0x800) {
				...
				for (i = 0; i < (savesize + 0x1FF) / 0x800; i++) {
					f_write(&file, pReadCache, 0x200 * 4,
						&written);
					if (written != 0x200 * 4)
						break;
				}
...
u8 Check_saveMODE(u8 gamecode[])
{
	u32 i;
	BYTE savemode = 0xFF;
	dmaCopy((void *)saveMODE_table, (void *)pReadCache,
		sizeof(saveMODE_table));
	for (i = 0; i < 3000; i++) {
		if (memcmp(((SAVE_MODE_ *) pReadCache)[i].gamecode, "FFFF", 4)
		    == 0) {
			break;
		} else
		    if (memcmp
			(((SAVE_MODE_ *) pReadCache)[i].gamecode, gamecode,
			 4) == 0) {
...
u32 IWRAM_CODE Loadfile2PSRAM(TCHAR * filename)
{
	...
	if (res == FR_OK) {
		...
		f_lseek(&gfile, 0x0000);
		for (blocknum = 0x0000; blocknum < filesize;
		     blocknum += 0x20000) {
			sprintf(msg, "%luMb", (blocknum) / 0x20000);
			Clear(78 + 54, 160 - 15, 110, 15, gl_color_cheat_black,
			      1);
			DrawHZText12(msg, 0, 78 + 54, 160 - 15, gl_color_text,
				     1);
			f_read(&gfile, pReadCache, 0x20000, &ret);	//pReadCache max 0x20000 Byte

			if ((gl_reset_on == 1) || (gl_rts_on == 1)
			    || (gl_sleep_on == 1) || (gl_cheat_on == 1)) {
				PatchInternal((u32 *) pReadCache, 0x20000,
					      blocknum);
			}
			...
			SetPSRampage(page);
			dmaCopy((void *)pReadCache, PSRAMBase_S98 + Address,
				0x20000);
			page = 0;
		}
		f_close(&gfile);
...
void CheckSwitch(void)
{
	...
	if ((gl_ingame_RTC_open_status != 0x0)
	...
	if (res == FR_OK) {
		...
		if (is_EMU == 3) {	//nes pocketnes_2013_07_01
			...
			*((vu32 *) pReadCache + 1) = 0x0;
			dmaCopy((void *)pReadCache,
				PSRAMBase_S98 + rom_start_address - 0x30, 0x8);
			*(vu32 *) pReadCache = filesize;
			dmaCopy((void *)pReadCache,
				PSRAMBase_S98 + rom_start_address - 0x10, 0x4);

			*(vu32 *) pReadCache = 0x709346c0;	//usr rtc
			dmaCopy((void *)pReadCache, PSRAMBase_S98 + 0x1EA0,
				0x4);
		...
		for (blocknum = 0x0000; blocknum < filesize;
			...
			SetPSRampage(page);
			dmaCopy((void *)pReadCache,
				PSRAMBase_S98 + rom_start_address + Address,
				0x20000);
	...
	if (folder_total > 1) {
		for (ret = 0; ret < folder_total - 1; ret++) {
			for (i = 0; i < folder_total - ret - 1; i++) {
				get =
				    strcmp(pFolder[i].filename,
					   pFolder[i + 1].filename);
				if (get > 0) {
					dmaCopy(&pFolder[i + 1],
						&(pFilename_temp.filename),
						sizeof(FM_Folder_FS));
					dmaCopy(&pFolder[i], &pFolder[i + 1],
						sizeof(FM_Folder_FS));
					dmaCopy(&(pFilename_temp.filename),
						&pFolder[i],
						sizeof(FM_Folder_FS));
				}
			}
	...
	if (game_total_SD > 1) {
		for (ret = 0; ret < game_total_SD - 1; ret++) {
			for (i = 0; i < game_total_SD - ret - 1; i++) {
				get =
				    strcmp(pFilename_buffer[i].filename,
					   pFilename_buffer[i + 1].filename);
				if (get > 0) {
					dmaCopy(&pFilename_buffer[i + 1],
						&pFilename_temp,
						sizeof(FM_FILE_FS));
					dmaCopy(&pFilename_buffer[i],
						&pFilename_buffer[i + 1],
						sizeof(FM_FILE_FS));
					dmaCopy(&pFilename_temp,
						&pFilename_buffer[i],
						sizeof(FM_FILE_FS));
				}
			}
...
u32 Load_Thumbnail(TCHAR * pfilename_pic)
{
	...
	if (res == FR_OK) {
		...
		sprintf(picpath, "/IMGS/%c/%c/%c%c%c%c.bmp", GAMECODE[0],
			GAMECODE[1], GAMECODE[0], GAMECODE[1], GAMECODE[2],
			GAMECODE[3]);
...
void SD_list_L_START(show_offset, file_select, folder_total)
{
	...
	while (1) {
		...
		if (keysdown & KEY_A) {
			TCHAR *pdelfilename;
			pdelfilename =
			    pFilename_buffer[show_offset + file_select -
					     folder_total].filename;
	...
	if (page_num == SD_list) {
		...
		if (res == FR_OK) {
			while (1) {
				...
				{
					memcpy(pFolder[folder_total].filename,
					       fileinfo.fname, 100);
					pFolder[folder_total++].filename[99] =
					    0;
					if (folder_total > MAX_folder)	//cut
						break;
				} else if (fileinfo.fattrib == AM_ARC) {
					memcpy(pFilename_buffer[game_total_SD].
					       filename, fileinfo.fname, 100);
					pFilename_buffer[game_total_SD].
					    filename[99] = 0;
					pFilename_buffer[game_total_SD++].
...
//SD_list:
	...
	while (1) {
		while (1)	//2
		{
			...
			if (updata && gl_show_Thumbnail) {
				...
				if (!strcasecmp
				    (&(pfilename_pic[strlengba - 3]), "gba")) {
					is_GBA = 1;
					haveThumbnail =
					    Load_Thumbnail(pfilename_pic);
					short_filename = 1;
				} else {
			...
			if (updata == 1) {	//reshow all
				if (page_num == SD_list) {
					...
					ClearWithBG((u16 *) gImage_SD, 0, 20,
						    240, 160 - 20, 1);
					Show_ICON_filename(show_offset,
							   file_select,
							   gl_show_Thumbnail
							   && is_GBA);
				...
				{
					DrawPic((u16 *) gImage_SET, 0, 0, 240,
						160, 0, 0, 1);
					res = Setting_window();
					if (res == 0) {
						DrawPic((u16 *) gImage_NOR, 0,
							0, 240, 160, 0, 0, 1);
						page_num = NOR_list;	//NOR
					} else {
						DrawPic((u16 *) gImage_HELP, 0,
							0, 240, 160, 0, 0, 1);
						page_num = HELP;	//HELP
					}
					goto re_showfile;
				...
				{
					DrawPic((u16 *) gImage_HELP, 0, 0, 240,
						160, 0, 0, 1);
					Show_help_window();
					DrawPic((u16 *) gImage_SET, 0, 0, 240,
						160, 0, 0, 1);
					...
					DrawPic((u16 *) gImage_NOR, 0, 0, 240,
						160, 0, 0, 1);
					...
					Show_ICON_filename_NOR(show_offset,
							       file_select);
				}
				Show_game_num(file_select + show_offset + 1,
					      page_num);
			} else if (updata > 1) {
				if (page_num == NOR_list) {
					Refresh_filename_NOR(show_offset,
							     file_select,
							     updata);
					ClearWithBG(gImage_NOR, 185, 0, 30, 18,
						    1);
				} else {
					Refresh_filename(show_offset,
							 file_select, updata,
							 gl_show_Thumbnail
							 && is_GBA);
					ClearWithBG((u16 *) gImage_SD, 185, 0,
						    30, 18, 1);
				}
				Show_game_num(file_select + show_offset + 1,
					      page_num);
			...
			if (updata && gl_show_Thumbnail && is_GBA
			    && (page_num == SD_list)) {
				if (haveThumbnail) {
			...
			if (page_num == SD_list) {
				if (game_folder_total)
					Filename_loop(shift, show_offset,
						      file_select,
						      short_filename);
			}
			...
			} else if (keysdown & KEY_B)	//return
			{
				if (page_num == SD_list) {
					...
					if (strcmp(currentpath, "/") != 0) {
						dmaCopy(currentpath,
							...
							MAX_path_len);
						TCHAR *p =
						    strrchr(currentpath_temp,
							    '/');
						memset(currentpath, 0x00,
						       MAX_path_len);
						strncpy(currentpath,
							currentpath_temp,
							p - currentpath_temp);
				...
				if (page_num == SD_list) {
					...
					if (show_offset + file_select <
						...
						p_folder_select_show_offset
						    [folder_select] =
						    show_offset;
						p_folder_select_file_select
						    [folder_select] =
						    file_select;
						folder_select++;
		...
		if (play_re == 0xBB) {
			if (page_num == SD_list) {
				...
				pfilename =
				    pNorFS[show_offset + file_select].filename;
			}
		} else {
			u8 *p = strrchr(p_recently_play[play_re], '/');
			strncpy(currentpath_temp, currentpath, 256);	//old
			memset(currentpath, 00, 256);
			strncpy(currentpath, p_recently_play[play_re],
				p - p_recently_play[play_re]);
			if (currentpath[0] == 0) {
				currentpath[0] = '/';
			}
 ...
 re_show_menu:
		...
		{
			if (re_menu) {
				Show_MENU(MENU_line, page_num,
					  ((havecht > 0) ? 1 : 0), Save_num,
					  is_EMU);
			}
			VBlankIntrWait();
		...
		DrawHZText12(gl_Loading, 0, (240 - strlen(gl_Loading) * 6) / 2,
			     74, gl_color_text, 1);
		...
		} else		//run nor game 
		{
			...
			gamefilesize =
			    pNorFS[show_offset + file_select].filesize;
			memcpy(GAMECODE,
			       &pNorFS[show_offset + file_select].gamename[0xC],
			       4);
		}
		...
		if (page_num == NOR_list) {	//boot nor game
			...
			if (pNorFS[show_offset + file_select].have_patch
			    && pNorFS[show_offset + file_select].have_RTS) {
			...
			Send_FATbuffer(FAT_table_buffer, 1);	//only save FAT
			//wait_btn();
			SetRompageWithHardReset(pNorFS
						[show_offset +
						 file_select].rompage, key_L);
			while (1) ;
		} else {
			switch (MENU_line) {
			...
			case 1:	//PSRAM BOOT WITH ADDON
				...
				} else	//(have_pat==0)
				{
					...
					res =
					    f_open(&gfile, pfilename, FA_READ);
					f_lseek(&gfile,
						(gamefilesize -
						 1) & 0xFFFE0000);
					f_read(&gfile, pReadCache, 0x20000,
					       &ret);
					f_close(&gfile);
					SetTrimSize(pReadCache, gamefilesize,
						    0x20000, 0x0, saveMODE);
			...
			case 2:	//WRITE TO NOR CLEAN         
				f_chdir(currentpath);	//return to game folder
				res =
				    Loadfile2NOR(pfilename, gl_norOffset, 0x0);
				if (res == 0) {
					page_num = NOR_list;
					goto refind_file;
				} else if (res == 2) {
					DrawHZText12("NOR FULL!", 0, 0,
						     160 - 15, gl_color_NORFULL,
						     1);
					wait_btn();
					goto refind_file;
				}
			...
			case 3:	//WRITE TO NOR ADDON
				...
				if ((gl_reset_on == 1) || (gl_rts_on == 1)
					...
					if (res == FR_OK) {
						f_lseek(&gfile,
							(gamefilesize -
							 1) & 0xFFFE0000);
						f_read(&gfile, pReadCache,
						       0x20000, &ret);
						f_close(&gfile);
						SetTrimSize(pReadCache,
							    gamefilesize,
							    0x20000, 0x1,
							    saveMODE);
					}
				...
				res =
				    Loadfile2NOR(pfilename, gl_norOffset,
						 needpatch);
				//wait_btn();   
				if (res == 0) {
					page_num = NOR_list;
