...
void Write(u32 romaddress, const u8 * buffer, u32 size)
{
	u32 x;
	if (is_NORpatch) {
		if ((romaddress >= windows_offset)
		    && (romaddress < windows_offset + 0x20000)) {
			for (x = 0; x < size / 2; x++) {
				((vu16 *) (pReadCache + romaddress -
					   windows_offset))[x] =
				    ((vu16 *) buffer)[x];
			}
	...
	if (index_DragonBallZ != -1) {
		...
		case 0:	//2in1 us
			for (u32 ii = 0; ii < sizeofa(patch_2_offsets); ii++) {
				Write(patch_2_offsets[ii] + 0x40000,
				      (u8 *) & patch, sizeof(patch));
			}
			patch = 0x1001;
		...
		case 1:	//I eu
			...
			for (u32 ii = 0; ii < sizeofa(patch_1_offsets); ii++) {
				Write(patch_1_offsets[ii], (u8 *) & patch,
				      sizeof(patch));
			}
			break;
		case 2:	//I us
			for (u32 ii = 0; ii < sizeofa(patch_2_offsets); ii++) {
				Write(patch_2_offsets[ii], (u8 *) & patch,
				      sizeof(patch));
			}
			break;
		case 3:	//II eu
	...
	if (0) {
		...
		for (u32 ii = bottom; ii >= top; ii--) {
			if (buffer[ii] != byte || ii == top) {
				iTrimSize = ii + 4;
				iTrimSize =
				    iTrimSize + ((romsize - 1) & 0xFFFE0000);
...
void Patch_B_address(void)
{
	...
	for (u32 ii = 0; ii < iCount2; ii++) {
		Write(iPatchInfo2[ii].iOffset * 4,
		      (u8 *) & (iPatchInfo2[ii].iValue),
...
void Patch_Reset_Sleep(u32 * Data)
{
	...
	dmaCopy((void *)p_patch_start, patchbuffer,
		p_patch_end - p_patch_start);
...
void Patch_RTS_Cheat(u32 * Data)
{
	...
	*(vu32 *) (patchbuffer + Return_address_offset) = Return_address;	//modify gba_sleep_patch_bin return address
	...
	if (spend_address != 0x0) {
		*(vu32 *) (patchbuffer + Return_address_offset + 4) =
		    spend_address;
	}
	...
	*(vu32 *) (patchbuffer + RTS_Reset_key_offset) =
	    RTS_reset_key_val & 0x3FF;
	if (gl_sleep_on != 1) {
		*(vu32 *) (patchbuffer + RTS_Sleep_key_offset) = 0;
	} else {
		*(vu32 *) (patchbuffer + RTS_Sleep_key_offset) =
		    RTS_sleep_key_val & 0x3FF;
	}
	...
	*(vu32 *) (patchbuffer + cheat_count_offset) = gl_cheat_count;
	...
	for (u32 ii = 0; ii < gl_cheat_count; ii++) {
		if (pCHEAT[ii].address >= 0x40000) {
			*(vu32 *) (patchbuffer + cheat_offset + 8 * ii) =
			    ((pCHEAT[ii].address) & 0x7FFF) + 0x3000000;
		} else {
			*(vu32 *) (patchbuffer + cheat_offset + 8 * ii) =
			    ((pCHEAT[ii].address) & 0x3FFFF) + 0x2000000;
		}
		*(vu32 *) (patchbuffer + cheat_offset + 8 * ii + 4) =
		    pCHEAT[ii].VAL;
...
void Patch_RTS_only(u32 * Data)
{
	...
	*(vu32 *) (patchbuffer + Return_address_offset) = Return_address;	//modify gba_sleep_patch_bin return address
	...
	u16 read9 = Read_SET_info(9);
	u16 read10 = Read_SET_info(10);
	u16 RTS_only_SAVE_key_val =
	    ~((1 << read5) | (1 << read6) | (1 << read7));
	u16 RTS_only_LOAD_key_val =
	    ~((1 << read8) | (1 << read9) | (1 << read10));
	...
	*(vu32 *) (patchbuffer + RTS_only_SAVE_key_offset) =
	    RTS_only_SAVE_key_val & 0x3FF;
	*(vu32 *) (patchbuffer + RTS_only_LOAD_key_offset) =
	    RTS_only_LOAD_key_val & 0x3FF;
...
u32 Get_spend_address(u32 * Data)
{
	...
	if ((Data[address / 4] >
	     0x03007E80) /*|| (Data[address/4] == 0x03007E00) */ ||(Data[address
									 / 4] ==
								    0x0203FFFC))
	{
...
void GBApatch_PSRAM(u32 * address, int filesize)	//Only once
{
	...
	if ((gl_rts_on == 1) && (gl_cheat_on == 0) && (gl_reset_on == 0)
	    && (gl_sleep_on == 0)) {
		spend_address = Get_spend_address(address);
		Patch_RTS_only(address);
	} else if ((gl_rts_on == 1)
		   || ((gl_cheat_on == 1) && (gl_cheat_count > 0))) {
		spend_address = Get_spend_address(address);
...
u32 Check_pat(TCHAR * gamefilename)
{
	...
	if (find_the_patfile) {
		...
		if ((w_reset_on != gl_reset_on) || (w_rts_on != gl_rts_on)
		    || (w_sleep_on != gl_sleep_on)
...
void Make_pat_file(TCHAR * gamefilename)
{
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			res =
			    f_write(&gfile, (void *)iPatchInfo2,
				    sizeof(iPatchInfo2), &written);
			...
			res =
			    f_write(&gfile, (void *)w_buffer, sizeof(w_buffer),
				    &written);
			f_close(&gfile);
		}
	}
	...
	} else			//make a new one
	{
		...
		if (res == FR_OK) {
			...
			for (i = 0; i < (0x70000) / 0x800; i++) {
				f_write(&gfile, pReadCache, 0x200 * 4,
					&written);
				if (written != 0x200 * 4)
					break;
			}
...
void Check_Fire_Emblem(void)
{
	...
	if (have) {
		...
		if (modify_val) {
			u32 p_modify_address_offset =
			    p_modify_address - p_patch_start;
			*(vu32 *) (patchbuffer + p_modify_address_offset) =
			    modify_val;
		}
